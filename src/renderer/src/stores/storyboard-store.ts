...
export const useStoryboardStore = create<StoryboardState>((set, get) => ({
  ...
  transitionModelId: 'pixverse-v6/transition',
  defaultDuration: 5,
  ...
  loadBoard: async (id) => {
    ...
    try {
      ...
      set({
        ...
        transitionModelId: result.transitionModelId ?? result.transition_model_id ?? 'pixverse-v6/transition',
        defaultDuration: result.defaultDuration ?? result.default_duration ?? 5,
        defaultAspectRatio: result.defaultAspectRatio ?? result.default_aspect_ratio ?? '16:9',
